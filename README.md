...
在桌面 QQ 里**按住 ⌥ 点一下对方的某条消息**，就用 [Jev](https://typesafe.ai)（TypeSafe AI 的判别模型）判断：
**她想表达的意思是不是这句话的字面意思？** 表里一致 → `YES`，话里有话 → `NO`，顺带给出最可能的真实意图。

结果贴在那个气泡旁边的浮窗里。新消息、错过的旧消息都一样点。程序不进 Dock，靠**菜单栏图标**（💬 气泡）确认它在跑：
图标旁会短暂显示最近一次的 YES / NO，点开菜单能看到正在看哪个聊天、打开设置、检查更新、退出。
（想让它自动判对方新发的每一句，见下文 `--auto`。）
...
之后打开 QQ 的聊天窗口，按住 ⌥ 点对方的消息就行。菜单栏图标里可以打开设置、检查更新、打开配置文件、打开日志（`~/Library/Logs/chat-jev.log`）、退出。
...
```bash
JEV_WATCH_ARGS="--auto --interval 0.5"     # 另外自动判对方新发的消息、读得勤一些
```
...
app 启动一分钟后、之后每 6 小时，会看一眼 GitHub 上的最新 release。有新版本就下载、换掉 `/Applications` 里的旧版、自动重启，
浮窗会提示「正在更新到 …」。也可以从菜单栏点「检查更新」手动查；设置窗口里可以关掉自动更新。

- **安全**：新版本必须和正在运行的版本是同一张证书签的，不是就拒绝安装。所以就算有人往 release 里塞了别的包，也装不进来。
- **权限不用重新给**：从 0.3.1 起用固定证书签名，系统把新旧版本认成同一个程序，辅助功能权限会保留。
- 换签名证书也能自动更新，见下文「签名证书」。
- **0.3.0 及更早的版本不能自动更新**（那时是 ad-hoc 签名）：手动下载一次 0.3.1，在辅助功能列表里用「−」删掉旧的 chat-jev，
  再打开新版重新授权。之后的版本就都能自动更新了。
- 自动下载的更新不会被「Apple 无法验证」拦下，只有从浏览器下载的 DMG 才会。
...
app 做的就是 `watch --app qq`：只判 ⌥+点击的消息。下面这些是从源码跑的命令：
...
# 另外自动判对方新发的每一句（默认关）
uv run chat-jev watch --auto

# 不要浮窗，只打终端（只能配合 --auto）
uv run chat-jev watch --auto --no-overlay
...
`watch` 启动时浮窗会弹一下"chat-jev 已启动"，菜单栏出现气泡图标。它一直在后台读当前聊天窗口，把读到的消息攒成上下文，
但**默认不自动判**，只判你 ⌥+点击的那条。

加了 `--auto` 才会自动判**对方新发的**消息（自己发的只记进上下文，切换聊天对象时上下文分开记）。
来回滑动聊天窗口时，出现过的消息不会被当成新消息再判一遍；一次冒出超过 5 条没见过的消息，会被当成快速滑动跳过了一段，也不判。
...
- 只判文字。撤回的消息不处理。
- `--auto` 模式下，对方连着两轮发了一模一样的话（前一句也一样），第二次会被当成重复跳过，要判就 ⌥+点击。
...
app 用一张固定的自签名证书签名，放在仓库外的 `~/.chat-jev-signing/`（钥匙串 + 密码，别提交）。
系统和自动更新都按它认 app：同一张证书签的新版本，自动更新会接受，辅助功能权限也保留。

- **备份这个目录**（U 盘、密码管理器附件都行）。换电脑打包就把它拷过去。
- `packaging/certs.txt` 钉死了该用哪张证书（`sign` 行）。证书目录不在、或者拿错了备份，`build.sh` 会直接报错停下，
  不会悄悄生成新证书，那样已装的 app 就会全部收不到更新。

**想换证书、旧证书还在**：用两步交接，已装的 app 不用重装。

```bash
packaging/rotate-cert.sh prepare   # 生成新证书，certs.txt 里加一行 trust
# 提交，照常发一版（还是旧证书签的）。已装的 app 自动更新到这版后，就认新证书了
packaging/rotate-cert.sh switch    # 换成新证书签名（没发过交接版会拒绝执行）
# 提交，以后照常发版
```

换完之后辅助功能权限会失效一次，因为系统按证书认程序，这一步 macOS 不允许程序代劳。
app 启动时发现没权限，会自动清掉旧的授权记录（不然开关看着开着其实无效），再打开设置页，拨一下开关就行，不用重装。

**证书真的丢了**：没有安全的自动办法。已装的 app 只装信任的证书签的包，这把锁正是用来挡假包的。
只能删掉 `certs.txt` 的 `sign` 行，重新打包发版，每台装了的电脑手动装一次新版。所以一定要备份。
...
chat_jev/
  ...
  updater.py      自动更新：查 GitHub release、校验签名、换包重启
packaging/
  build.sh        打包 + 签名 + DMG
  signing.sh      证书相关的公共函数
  rotate-cert.sh  换签名证书（两步交接）
  certs.txt       钉死的签名证书 / 额外信任的证书
  menubar.py      菜单栏图标 + 菜单
  app.py          Watcher 主循环
  cli.py          命令行
```
